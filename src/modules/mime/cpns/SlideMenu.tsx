...
const ContentWidth = SCREEN_WIDTH * 0.7;

type Props = {};

function SlideMenu({}: Props, ref: Ref<any>) {
  ...
  const onMenuItemPress = useCallback(async (item: any) => {
    if (item.name === '退出登录') {
      hide();
        ...
        <View style={[styles.content, { marginLeft: open ? 0 : -ContentWidth }]}>
          ...
          <View style={styles.bottomLayout}>
            {BOTTOM_MENUS.map(item => {
              return (
                <TouchableOpacity key={`${item.txt}`} style={styles.bottomMenuItem}>
...
export default forwardRef(SlideMenu);
